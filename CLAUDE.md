...
- DB アクセスし状態を検証する
  1. `compose.yml` を確認し、ユーザ名、パスワード、DB 名を確認する
  2. DB コンテナを確認し `docker exec -c` で SQL を実行する
- Issue 作成
  1. 作成する Issue の内容を明確にする
  2. `.github/ISSUE_TEMPLATE/` フォルダにあるテンプレートを使用して Issue を作成する
- Pull Request 作成
  1. 作成する Pull Request の内容を明確にする
  2. `.github/PULL_REQUEST_TEMPLATE.md` を使用したテンプレートに従って Pull Request を作成する
- E2E テストの実施
  1. Docker コンテナが起動していることを確認する
  2. Next.js アプリケーションが起動していることを確認し、起動していなければ `npm run dev` で起動する
  3. Playwright を使用して、指示にあるテストを実施する
  4. 動作検証が完了したら、Next.js アプリケーションを停止する
